...
class BalloonsTwoPlayer:
    def __init__(self, clock, scene, level):
        ...
        self.step = [0, 0]
        self.length = 200
        self.balloon_speed_left = 5
        self.balloon_speed_right = 5
        self.time = 10
            ...
            if 550 <= x <= 550+170:
                if 500 <= y <= 500+60:
                    if self.level == 2:
                        print("关卡暂未开放")
                        self.scene.set_main_scene()
                    self.scene.scenes = BalloonsTwoPlayer(self.clock, self.scene, self.level+1)
                    self.scene.run_game()

    def reset_balloon_left(self):
        self.score[0] += 1
        self.balloon_speed_right += 1
        self.remove_balloon_left()
        self.set_balloon_speed_left()

    def reset_balloon_right(self):
        self.score[1] += 1
        self.balloon_speed_left += 1
        self.remove_balloon_right()
    ...
    def set_balloon_speed_left(self):
        if self.score[0] == 10:
            self.balloon_speed_left = 5 + 1
        elif self.score[0] == 20:
            self.balloon_speed_left = 5 + 1 + 0.8
        elif self.score[0] == 30:
            self.balloon_speed_left = 5 + 1 + 0.8 + 0.6
        elif self.score[0] == 40:
            self.balloon_speed_left = 5 + 1 + 0.8 + 0.6 + 0.4
        elif self.score[0] == 50:
            self.balloon_speed_left = 5 + 1 + 0.8 + 0.6 + 0.4 + 0.2

    def set_balloon_speed_right(self):
        if self.score[1] == 10:
            self.balloon_speed_right = 5 + 1
        elif self.score[1] == 20:
            self.balloon_speed_right = 5 + 1 + 0.8
        elif self.score[1] == 30:
            self.balloon_speed_right = 5 + 1 + 0.8 + 0.6
        elif self.score[1] == 40:
            self.balloon_speed_right = 5 + 1 + 0.8 + 0.6 + 0.4
        elif self.score[1] == 50:
            self.balloon_speed_right = 5 + 1 + 0.8 + 0.6 + 0.4 + 0.2
